...
function SaveToFile($path = "", $file, $s, $mode)
{
...
function MakeDefinitionFile($path= "", $file, $type, $enc, $embed, $subset, $map, $info)
{
	$s = "<?php\n";
	$s .= '$type = \''.$type."';\n";
	...
	$s .= "?>\n";
	SaveToFile($path, $file, $s, 't');
}

function MakeFont($fontfile,  $path = "", $enc='cp1252', $embed=true, $subset=true)
{
	...
	if($embed)
	{
		...
		{
			$file = $basename.'.z';
			SaveToFile($path, $file, gzcompress($info['Data']), 'b');
			$info['File'] = $file;
	...
	MakeDefinitionFile($path, $basename.'.php', $type, $enc, $embed, $subset, $map, $info);
